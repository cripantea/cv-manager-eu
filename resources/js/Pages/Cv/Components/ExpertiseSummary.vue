        ...
        <div v-if="sorted.length">
            ...
            <table class="min-w-full text-sm">
                ...
                <tbody class="divide-y divide-gray-100">
                    <tr v-for="[tech, months] in sorted" :key="tech" class="py-1">
                        <td class="py-2 text-gray-800 font-medium capitalize">{{ tech }}</td>
                        <td class="py-2 text-right text-gray-600 font-mono">{{ months }}</td>
                        <td class="py-2 pl-4">
                            <div class="h-2 bg-gray-100 rounded-full overflow-hidden">
                                <div
                                    class="h-full bg-blue-500 rounded-full"
                                    :style="{ width: barWidth(months) }"
                                />
                            </div>
                        </td>
                    </tr>
...
const maxMonths = computed(() => sorted.value[0]?.[1] ?? 1);
